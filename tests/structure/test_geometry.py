...
def test_dihedral_backbone_consistency(multi_model):
    ...
    MDTraj.
    """
    with open(data_dir("structure") / "misc" / "dihedrals.json") as file:
...
def test_nucleotide_dihedral_consistency(multi_model):
    ...
    """
    with open(data_dir("structure") / "misc" / "nucleotide_dihedrals.json") as file:
        ref_dihedrals = json.load(file)
...
def test_index_functions():
    ...
    indices = rng.integers(array.array_length(), size=(100, 4), dtype=int)
